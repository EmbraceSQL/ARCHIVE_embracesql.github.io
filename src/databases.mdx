...
EmbraceSQL not only works with multiple database flavors, it works with multiple databases. By default, there is one database named `default`, which points to the local sqlite. So -- no config at all is equivalent to:
...
Using the EmbraceSQL Docker, do a `reload`. This will create the new database, do an `ls` and see there will be two local db files.
