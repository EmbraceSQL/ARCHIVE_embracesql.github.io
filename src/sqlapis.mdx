...
To make batching simple, every SQL API and AutoCRUD call has automatic batch support. In addition to named parameters, and positional parameters, you can pass in an array -- a batch -- of parameters, which execute together in one shot. This is so automatic in fact, everything in EmbraceSQL is a batch -- it's just that some batches have only one entry.
...
- Request/Resposne via REST
- Request/Response via generated client libraries
- Fire and Forget via messages

### REST

Every SQL API and AutoCRUD is exposed via REST, as shown in examples. These 'block' until complete and return to the caller, transactions roll back automatically if any timeout occurs.
...
Parameters can be positional (array), named (hash), batch (array of arrays or hash), or batch (path of file).
