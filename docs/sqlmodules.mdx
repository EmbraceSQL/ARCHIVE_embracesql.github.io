...
```shell
docker run embracesql/server migrate
```
...
To make batching simple, every SQL module call has automatic batch support.
...
- You submit a batch to a SQL module, by value as a string, or by reference as a path to a file
- If by reference, the file is downloaded for you into a batch
- A transaction is started for you
- For each row in the batch, run the SQL module
