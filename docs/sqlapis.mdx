...
```javascript title="./things/all.sql.before.js"
export const before = async (context) => {
...
```javascript title="./other/before.js"
export const before = async (context) => {
...
Working on single sets of parameters is great, but sometimes you want to do the same set of operations to an array of data. This is where batching comes in.

To make batching simple, every SQL API and AutoCRUD call has automatic batch support.
In addition to named parameters, you can pass in an array -- a batch -- of parameters, which execute together in one shot.
This is so automatic in fact, everything in EmbraceSQL is a batch -- it's just that when you send in named parameters _not_ in an array, it is a 'batch of one'.

The semantics of the batch are:

- You submit a batch to a SQL API or AutoCRUD, by value as a string, or by reference as a path to a file
- If by reference, the file is downloaded for you into a batch
- A transaction is started for you
- For each row in the batch, run the SQL API or AutoCRUD
- For each exception, raise an event as normal
- For each success, buffer in a results collection
- Commit the transaction started for you if no unhandled errors

### Error Handling

Some errors are more critical than others. Sometimes you want to keep going, sometimes you want to abort. EmbraceSQL is a bit of a perfectionist, and aborts the entire batch on any unhandled error. You can be more forgiving though, and decide to keep going by creating an error handler:
...
export const error = (context) => {
  ...
  // raise it again -- or any error and the batch aborts
  // throw "Oh! Noes!";
};
```
...
## Invoking SQL APIs and AutoCRUDs
...
Every SQL API and AutoCRUD is exposed via REST, as shown in examples.
...
To avoid timeouts, and to not wait on long jobs you can submit a request via SQS, which you configure:

```yaml title="./embracesql.yml"
fire_and_forget_from: amazon://${AWS_ACCESS_KEY_ID}:${AWS_SECRET_ACCESS_KEY}/arn:aws:sqs:us-east-2:123456789012:queue1
fire_and_forget_workers: 1
```

Each message is on the queue is like:
...
- Named (name value pairs, JSON object)
- Batch (array of Named)
- Batch (path of file, string).

It's perfectly possible to have multiple EmbraceSQL servers running all pointed at the same input queue, in which case each server can and will be processing at least one message at a time.
You should consider using a [FIFO Queue](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/FIFO-queues.html).
