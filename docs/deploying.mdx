...
This will take the core EmbraceSQL server container, and overlay the current directoy into a new Docker image. From there you can push your image to any container registry or hosting tool you like, as well as convert from Docker to other container formats.
...
Even smarter -- EmbraceSQL, for each database, for each schema, keeps track of the schema after each migration and will warn you if the schema does no match the expectations, and raise and event containing the schema diff. Embrace SQL will still try to run your SQL APIs and AutoCRUD even if there is a mismatch, but having the mismatch schema output will greatly aid your comfort with versioning.
